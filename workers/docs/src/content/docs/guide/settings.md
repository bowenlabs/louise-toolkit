---
title: Louise Settings
description: The package-provided, registry-driven settings surface for structured, back-office editing.
sidebar:
  order: 8
---
...
Louise Settings opens from the edit bar's **Settings** button, which dispatches the
`louise:open-settings` event `mountLouise` fires. Mount both in your edit-mode
entry point:
...
`mountSettings` injects the shared stylesheet, creates one `QueryClient` for every
...
A `CollectionTab`'s `panel` is any Solid component. Reuse the Louise Settings
[data layer](/reference/client/#louiseclientsettings) and the shared field
