...
export const pagesCollection = defineCollection({
  ...
  // EDIT_SESSION binding is provisioned; otherwise the route returns 503 and the
  // editor falls back to the debounced-fetch auto-save.
  realtime: true,
  // Full-text search over the page title, body, and (flattened) sections
  // content—indexed into a `pages_fts` FTS5 table, kept in sync on publish.
