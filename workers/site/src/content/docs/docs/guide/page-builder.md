...
`@louisecms/core/client`'s `blocks` module holds the framework.
...
```ts
import { BLOCKS, BlockInserter } from "@louisecms/core/client";
```
...
```ts
import { defineBlock } from "@louisecms/core/client";
