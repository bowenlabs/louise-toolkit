---
title: cms
description: "@louisecms/core/cms — collections, codegen, the Local API, validation, patches, webhooks."
sidebar:
  order: 2
---
...
```ts
import { defineCollection, defineCmsConfig } from "@louisecms/core/cms";
...
```ts
import { cmsConfigToSchema, generateSchemaSource } from "@louisecms/core/cms";
```
...
```ts
import { createLocalApi, createVersionedLocalApi } from "@louisecms/core/cms";
...
```ts
import { rule } from "@louisecms/core/cms";
...
- `renderRichText(content)` — render stored rich-text content to HTML (the
  server never runs ProseMirror; see [Rich text](/docs/guide/rich-text/)).
