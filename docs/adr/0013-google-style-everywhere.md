...
**One lint runner, shipped in the package.** `vale/package/styles/Louise/lint-docs.mjs` collects files from git and runs Vale the same way in every repository, and `vale sync` delivers it to `.vale/Louise/lint-docs.mjs`, so a repository's `lint:docs` is one line. It covers the two file types Vale 3.17 can't parse. A `.mjs` file is linted as JavaScript, because as plain text Vale would lint its code and strings too. An `.astro` file is split into the text a visitor reads, linted as HTML, and its code (the frontmatter and any multi-line `{…}` block), linted as TypeScript so only comments count; both copies keep the original line numbers. The runner is a plain gate by default and a per-file ratchet with `--baseline`.

**Separators aren't dashes.** Linting `.astro` templates surfaced spaced dashes used as separators rather than punctuation: page titles (`Overview — Example Organization admin`), testimonial attributions, and joined spec lines. Unspacing them would have changed visible titles and layouts on every client site, so separators get their own characters instead. Page titles use a pipe (`Overview | Example Organization admin`), and items side by side use a middle dot (`Jane Doe · Founder`). A dash is only for sentences.
