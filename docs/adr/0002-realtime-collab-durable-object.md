...
server-side draft-merge narrows the window, but there's no live channel between clients, no
...
over a hibernatable WebSocket; the DO owns authoritative in-memory field state and a presence
...
existing debounced-fetch auto-save (#68) and KV buffer (#70) remain the **fallback** for
...
the same #69 D1 session. There's **one write path**; the DO is a new _front end_ to it, not a
...
- **Rich-text body** (ProseMirror / the `[...slug]` body): LWW on a whole-document field is
  ...
  focus the body claims it (`claim`), and peers see it read-only with a "locked by Baylee" badge
  until `release`/disconnect. This is the v1 answer for the one field where character-level merge
  matters.

**Why not a CRDT now:** Yjs/Automerge would add a substantial dependency and rich-text
merge/transport complexity for a tool whose common case is _two editors on different fields_.
Field-level LWW, presence, and a rich-text soft-lock cover that case and make conflicts
...
- **Server/codegen:** a `realtime: true` option on the collection config, sibling to
  `versions.drafts` (realtime **requires** drafts, because the DO persists as drafts). This gates
  the DO route and any codegen.
...
The DO holds authoritative field state and a `Map<socket, presence>`; it validates every
...
### 5. Lifecycle, hibernation, and persistence
...
- **Presence across hibernation:** stash each connection's identity with
  ...
  `this.ctx.getWebSockets()` + `deserializeAttachment()` on wake; nothing is held in memory only.
- **Coalesced flush:** on the first dirtying `change`, schedule a flush via
  ...
  `setTimeout` wouldn't). The `alarm()` handler calls **`applySaveDraft`** with the coalesced
...
- The WebSocket **upgrade request** passes the **same guard as the editor routes**,
  `resolveEditor` plus the same-origin check (`guardEditor`), _before_ `acceptWebSocket`. An
...
- The DO is addressed by `EDIT_SESSION.idFromName(`${slug}:${id}`)`; the `{slug,id}` are
  validated and authorized server-side (mirroring `versionsRoute`), so there's no cross-page
  reach.
...
1. **DO skeleton + infra** (task 1): `EditSessionDO` with hibernatable accept, the authenticated
...
2. **Presence + change-broadcast protocol** (task 2): the envelope from §4, authoritative field
   state, presence map (and hibernation attachment), and the presence UI in the edit chrome
...
DOs deploy with the Worker (no separate provisioning call). Unlike D1 replication (#69)
or Queues (#77), there's no pre-merge dashboard/API step: the binding and migration ship in the
...
- One write path: the DO reuses `applySaveDraft`, so drafts/versions/publish/#69 sessions all
  keep working, with no second store to reconcile.
...
- **CRDT-first (Yjs/Automerge through the DO).** Best-in-class rich-text merge, but a large
  dependency and transport/complexity cost for a mostly-different-fields workload. Deferred as the
  body-field upgrade path, not the v1 baseline.
- **Third-party realtime (Liveblocks / PartyKit / Ably).** Faster to presence, but adds an
  external dependency, cost, and data egress, and contradicts ADR 0001's Cloudflare-native core
...
- **Keep polling / short-poll the drafts endpoint.** No new primitive, but no true presence, no
  live cursors, and it reintroduces the write-volume problem #70 fixed. Rejected.
