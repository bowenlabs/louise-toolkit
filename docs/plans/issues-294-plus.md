...
first wave that follows is therefore not implementation; it's reconciling the record,
...
| 296 | `location_overrides` + presence       | ●◐  | Read _and_ write shipped: `SquarePresence` (`:347`), `SquareLocationOverride` (`:356`), `presentAt`, `priceAtLocation`, `overridesBody`, `presenceBody`. All three presence lists are there. **Residue:** the guard rail the issue asks for (assert a variation's location set ⊆ its parent item's) isn't implemented, and the "can a `#temp` variation carry `location_overrides` in the same batch" question is unanswered. |
...
| #   | Issue                                   |     | State in `main`                                                                                                                                                                                                                                                                   |
| --- | --------------------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 303 | `pos` commerce role                     | ●   | In `commerce/roles.ts` and threaded through secrets and status. Close it.                                                                                                                                                                                                         |
...
| 334 | one flag to turn off AI generation  | ○   | Four accessors still `ai: (env) => env.AI`.                                                                                                                            |
...
- **Close with a note:** #298 (the `TRANSFER` helper is explicitly optional in
  the issue's own text), #296 and #297 (see the residue that follows; file it, don't
...
- **#302: one decision.** The implementation defaults retry OFF; the issue asked
  ...
  unattended pushes actually run, and it keeps a hand-wired caller's behavior
  predictable. One line in `astroid/src/queues/scaffold.ts`; then close #302.
- **Amend #327's audit**: strike the internal-path-leak bullet, and note it landed
  ahead of the epic.
...
like ten unstarted features when it's really four.

---

## 4. Wave 1: cut the A2 release (#349 → #348 → #341)

The highest-value thing available today, and it's mostly a release chore.
...
3. Publish, then **deprecate `louise-toolkit@0.21.0` on npm** with
...
4. `reference/realtime.md`: the ownership split (the site owns the
   ...
   Cross-link ADR 0002.
5. `reference/qr.md`: encoder + SVG, why it's vendored, and the PNG path
...
**#314**, the solid-table guidance as a standing constraint: `flexRender` is fine
...
- **#294 first.** It's the only issue here with a live production failure mode:
...
- **#301 + #296r + #297r as one PR.** All three are the same shape, making the
  ...
  250-variation assert. The round-trip test #301 asks for isn't optional; the
...
full-page administration PWA. Sequencing matters more here than anywhere else, because
...
- **#313** the form scaffold: confirm the module referenced at
  ...
  both `validateSubmission` and `@tanstack/solid-form` is a good design that's
  currently invisible.
- **#305** and **#306** are storefront-side and parallel to all the preceding items.
- **#310** has no dependents and no dependencies. Slot it wherever there's
...
1. **`louise-toolkit/astro` moves into `astroidjs`**, not a separate
   ...
   doesn't exist isn't worth a package to maintain; extract one when a second
   host is real.
...
3. **The non-Astro consumer fixture is worth it, and it's nearly free**: the
...
4. **The client lifecycle seam last of the Phase-1 items**: replacing the
   ...
   `onNavigate`/teardown hook astroid wires up. It's the only Phase-1 item with
...
- **#296's open verification** (does a `#temp` variation accept
  ...
  can't be answered from source, and both determine whether onboarding a
