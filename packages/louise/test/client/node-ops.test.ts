...
function page(n: number, blocksPer = 0): HTMLElement {
  ...
  for (let i = 0; i < n; i++) {
    ...
    heading.setAttribute("data-louise-sfield", `${i}.heading`);
    sec.appendChild(heading);
    for (let j = 0; j < blocksPer; j++) {
      const b = document.createElement("div");
      b.setAttribute("data-louise-node", `${i}.blocks.${j}`);
      const f = document.createElement("span");
      f.setAttribute("data-louise-sfield", `${i}.blocks.${j}.body`);
      b.appendChild(f);
      sec.appendChild(b);
    }
...
const sfields = (root: Element) =>
  [...root.querySelectorAll("[data-louise-sfield]")].map((e) =>
    e.getAttribute("data-louise-sfield"),
  );
...
describe("moveNodeElement", () => {
  it("reorders sections and re-stamps every marker family", () => {
    const root = page(3);
    moveNodeElement([], 2, 0, root);

    expect(nodes(root)).toEqual(["0", "1", "2"]);
    // The heading that was section 2's is now section 0's — and its sfield must
    // have followed, or an edit to it writes into the wrong item.
  ...
  it("carries a section's blocks with it", () => {
    const root = page(2, 1);
    const movedBlockField = root.querySelectorAll('[data-louise-sfield="1.blocks.0.body"]')[0];
    moveNodeElement([], 1, 0, root);

    // Same element, new address — the whole subtree was re-stamped by prefix.
    expect(movedBlockField.getAttribute("data-louise-sfield")).toBe("0.blocks.0.body");
  });
...
describe("replaceNodeElement", () => {
  it("swaps a section in place and re-stamps only it", () => {
    ...
    const f = document.createElement("span");
    f.setAttribute("data-louise-sfield", "0.body");
    el.appendChild(f);
    ...
    expect(f.getAttribute("data-louise-sfield")).toBe("1.body");
  });
