...
// (for example, `"auth_"`) to render a visible auth boundary in one database. The SAME
...
export interface AuthSchemaConfig {
  ...
  customers?: boolean;
  /** Extra Better Auth user columns (for example, `squareCustomerId`). MUST match the
   ...
   *  runtime-only knobs (for example, `allowUserToCreateOrganization`) are omitted. */
  organizations?: { teams?: boolean };
  /** Table-name prefix for a same-D1 auth boundary (Option B), for example, `"auth_"`.
...
export function authSchemaOptions(config: AuthSchemaConfig): BetterAuthOptions {
  return {
    ...
    // Louise's standard first/last name fields, ahead of the site's own extras—mirrors
    // auth.ts so the generated schema matches the runtime user table.
