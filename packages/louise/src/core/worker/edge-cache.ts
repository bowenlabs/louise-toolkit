...
// requests always run the handler fresh. Crucially it STRIPS the
// `Cloudflare-CDN-Cache-Control` signal from every response, so the cookie-blind
// automatic edge cache never engages — this cache is the only one, and it
// excludes editors by construction.
//
...
export function withEdgeCache<Env = unknown>(
  ...
  return async (request, env, ctx) => {
    ...
    if (key && isCacheableDirective(directive)) {
      ...
      // directive onto it. Drop Set-Cookie — cache.put rejects a response that
      // carries one, and a shared public page must not ship a per-request cookie.
