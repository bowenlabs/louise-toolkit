 ...
 * `blocks.allow` when declared, otherwise the whole catalog (ADR 0005 §4—`allow`
 * omitted means "any block type"), with types the catalog doesn't know
 ...
 *
 * Returns `null` for any path that doesn't address something the editor knows—a
 * stale marker left by a fragment swap, a hand-written one, or a field the
