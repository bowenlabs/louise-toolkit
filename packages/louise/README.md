...
whole surface, not only content. Editing the live page in place is the headline
(no separate administration app, no JSON forms for prose), but it's one of ~two dozen
...
| `louise-toolkit/content/define`                       | The drizzle-free half of `content`: `defineCollection` + field/collection types, for describing content without the DB layer |
...
| `louise-toolkit/auth`                                 | Better Auth factory + guard/handler + `generateAuthSchemaSql` (and the `louise` command-line tool)                           |
| `louise-toolkit/security`                             | `sanitize`, rate-limit, secrets, security headers                                                                            |
| `louise-toolkit/worker`                               | `composeWorker` to compose editor + site routes over an SSR fallthrough; `withEdgeCache`, `withHealing`                      |
...
| `louise-toolkit/workflows`                            | Cloudflare Workflows helpers: durable, resumable multi-step pipelines (for example, the publish pipeline)                    |
