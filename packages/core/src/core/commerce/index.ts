// louise/commerce — Stripe glue, ported from the preview branch
// (workers/site/src/louise/commerce.ts). Raw fetch + crypto.subtle only — no
// Node SDKs. Adapted for the themidwestartist checkout: an embedded Payment Element
// over a multi-item cart (PaymentIntent) instead of single-item hosted
// Checkout Sessions; invoice + webhook-signature logic unchanged. Merch
// fulfillment moved off Printful to Fourthwall (Cycle 6) — see louise/commerce/fourthwall.
